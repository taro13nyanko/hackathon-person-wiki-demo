...
<!-- 2026-07-29: ビジュアルモード用の丸みのあるフォント（M PLUS Rounded 1c）をGoogle Fontsから読み込む。
     読み込み失敗時・オフライン時はCSS側のシステムフォントへ自動フォールバックする。 -->
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=M+PLUS+Rounded+1c:wght@400;500;700&display=swap" rel="stylesheet">
<script>
// 2026-07-29: ビジュアルモードのFOUC（一瞬だけ標準テーマが表示されてから切り替わる現象）を防ぐため、
// CSSより前・<body>描画より前に判定してhtml要素へclassを付与する（従来のダークモードを廃止し置き換え）。
(function(){
  try {
    var pref = localStorage.getItem('wikiVisual');
    if(pref === '1') document.documentElement.classList.add('visual');
  } catch(e){}
})();
</script>
<style>
...
<div id="layout">
  <div id="sidebar">
    <h2><span class="wiki-title-text" onclick="goHome()" title="ホームへ戻る">人物Wiki</span>__LAST_UPDATE_BADGE__<span class="wiki-title-actions"><button id="visualToggle" onclick="toggleVisualMode()" title="ビジュアルモード切替">🎨</button><button id="menuToggle" onclick="toggleSidebar()">絞り込み</button></span></h2>
...
function applyVisualModeIcon(){
  const btn = document.getElementById("visualToggle");
  if(!btn) return;
  const on = document.documentElement.classList.contains("visual");
  btn.classList.toggle("active", on);
  btn.title = on ? "標準モードに切り替え" : "ビジュアルモードに切り替え";
}
function toggleVisualMode(){
  document.documentElement.classList.toggle("visual");
  try { localStorage.setItem("wikiVisual", document.documentElement.classList.contains("visual") ? "1" : "0"); } catch(e){}
  applyVisualModeIcon();
}
...
function showArticle(id){
  ...
  // 2026-07-26: 逆リンク（このページを[[wikilink]]で参照している他のページ）。
  // 名前順（五十音順）に揃えて表示する。
  const backlinkRecs = (r.backlinks || []).map(bid => byId[bid]).filter(Boolean)
    .sort((a,b) => a.title.localeCompare(b.title, 'ja'));
  const backlinksHtml = backlinkRecs.length
    ? `<h3>このページを参照しているページ（${backlinkRecs.length}件）</h3><div id="backlinks">${
        backlinkRecs.map(b => `<a class="wikilink" onclick="showArticle('${b.id.replace(/'/g,"\\'")}')">${escapeHtml(dispName(b.title))}</a>`).join("")
      }</div>`
    : "";
